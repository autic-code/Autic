...
export async function buildCommand(
  options: { watch?: boolean; clean?: boolean } = {},
): Promise<void> {
  ...
  try {
    if (options.clean) {
      process.stdout.write(
        `  ${colorText('⟳', 'warning')} ${colorText('Cleaning previous builds...', 'dim')}`,
      );
      execSync('pnpm -r clean', { stdio: 'pipe' });
      process.stdout.write(
        `\r  ${colorText('✓', 'success')} ${colorText('Clean complete', 'success')}\n\n`,
      );
    } else {
      process.stdout.write(
        `  ${colorText('⟳', 'primary')} ${colorText('Compiling TypeScript', 'dim')}`,
      );
    }

    if (options.clean) {
      process.stdout.write(
        `  ${colorText('⟳', 'primary')} ${colorText('Compiling TypeScript', 'dim')}${options.watch ? ' (watch mode)' : ''}...\n`,
      );
    }
