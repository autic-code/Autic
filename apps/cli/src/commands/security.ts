...
export async function securityCommand(
  ...
  switch (effectiveAction) {
    case 'status': {
      ...
      console.log(
        `  ${theme.colors.textDim}Trust Profile:${theme.colors.text}  ${colorForLevel(profile.getProfile())}${profile.getProfile().replace('_', ' ').toUpperCase()}${theme.colors.text}`,
      );
      console.log(
        `  ${theme.colors.textDim}Auto Mode:${theme.colors.text}     ${formatMode(mode.getMode())}`,
      );
      console.log(
        `  ${theme.colors.textDim}Vault:${theme.colors.text}         ${vault.isInitialized() ? `${theme.colors.success}Initialized${theme.colors.text} (${vaultKeys.length} key(s))` : `${theme.colors.warning}Not initialized${theme.colors.text}`}`,
      );
      console.log(
        `  ${theme.colors.textDim}Events:${theme.colors.text}        ${events.size} recorded`,
      );
      console.log(
        `  ${theme.colors.textDim}Permissions:${theme.colors.text}   ${perms.getRules().length} rule(s)\n`,
      );
      ...
      if (summary.total > 0) {
        ...
        console.log(
          `  ${theme.colors.textDim}  Warnings:${theme.colors.text}  ${summary.warnings}`,
        );
        console.log(
          `  ${theme.colors.textDim}  Critical:${theme.colors.text}  ${summary.critical}\n`,
        );
      }
    ...
    case 'profile': {
      ...
      if (sub && ['safe', 'balanced', 'full_auto'].includes(sub)) {
        ...
        console.log(
          `  ${theme.colors.textDim}Profile:${theme.colors.text}        ${colorForLevel(profile.getProfile())}${profile.getProfile().replace('_', ' ').toUpperCase()}${theme.colors.text}`,
        );
        console.log(
          `  ${theme.colors.textDim}Dangerous Commands:${theme.colors.text} ${current.allowDangerousCommands ? `${theme.colors.error}Allowed${theme.colors.text}` : `${theme.colors.success}Blocked${theme.colors.text}`}`,
        );
        console.log(
          `  ${theme.colors.textDim}Medium Risk Approval:${theme.colors.text}  ${current.requireApprovalForMedium ? `${theme.colors.warning}Required${theme.colors.text}` : `${theme.colors.success}Auto${theme.colors.text}`}`,
        );
        console.log(
          `  ${theme.colors.textDim}Max Depth:${theme.colors.text}      ${current.maxExecutionDepth}`,
        );
        console.log(
          `  ${theme.colors.textDim}Timeout:${theme.colors.text}        ${current.workflowTimeoutMs}ms\n`,
        );
        ...
        for (const desc of descriptions) {
          ...
          console.log(
            `  ${c}${theme.icon.dot}${theme.colors.text} ${desc.name.replace('_', ' ').toUpperCase()} — ${desc.description}`,
          );
        }
        console.log(
    ...
    case 'permissions': {
      ...
      if (sub === 'list' || !sub) {
        ...
        if (rules.length === 0) {
          console.log(
            `  ${theme.colors.textDim}No permission rules configured.${theme.colors.text}\n`,
          );
        } else {
          for (const rule of rules) {
            ...
            console.log(
              `  ${color}${icon}${theme.colors.text} ${rule.action} on ${rule.resource} ${color}${rule.effect}${theme.colors.text} ${levelColor}(${level})${theme.colors.text}`,
            );
          }
          console.log();
        }
    ...
    case 'events': {
      ...
      if (allEvents.length === 0) {
        ...
        for (const event of allEvents) {
          ...
          console.log(
            `  ${sevColor}${theme.icon.dot}${theme.colors.text} [${time}] ${event.message}`,
          );
        }
        console.log();
      }
    ...
    case 'vault': {
      ...
      if (!vault.isInitialized()) {
        console.log(
          `  ${theme.colors.warning}Vault not initialized. Set AUTIC_VAULT_KEY or run init.${theme.colors.text}\n`,
        );
      } else {
        const keys = await vault.listKeys();
        console.log(
          `  ${theme.colors.textDim}Status:${theme.colors.text}  ${theme.colors.success}Initialized${theme.colors.text}`,
        );
        ...
        if (keys.length > 0) {
          ...
          for (const entry of keys.slice(0, 10)) {
            ...
            console.log(
              `  ${theme.icon.dot} ${entry.key}: ${theme.colors.textDim}${prefix || '[unknown]'}${theme.colors.text}`,
            );
          }
          if (keys.length > 10) {
            console.log(
              `  ${theme.colors.textDim}  ... and ${keys.length - 10} more${theme.colors.text}`,
            );
          }
          console.log();
        }
    ...
    default: {
      ...
      console.log(
        `  ${theme.colors.textDim}autic security${theme.colors.text}               Show security status`,
      );
      console.log(
        `  ${theme.colors.textDim}autic security status${theme.colors.text}         Show detailed security status`,
      );
      console.log(
        `  ${theme.colors.textDim}autic security profile${theme.colors.text}        Show current trust profile`,
      );
      console.log(
        `  ${theme.colors.textDim}autic security profile <name>${theme.colors.text}  Set trust profile (safe/balanced/full_auto)`,
      );
      console.log(
        `  ${theme.colors.textDim}autic security permissions${theme.colors.text}     List permission rules`,
      );
      console.log(
        `  ${theme.colors.textDim}autic security events${theme.colors.text}         Show recent security events`,
      );
      console.log(
        `  ${theme.colors.textDim}autic security vault${theme.colors.text}          Show vault status\n`,
      );
      break;
    }
  }
