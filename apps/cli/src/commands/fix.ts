...
export async function fixCommand(options: {
  ...
  if (options.dryRun) console.log(`  ${colorText('Mode:', 'dim')}    ${colorText('dry-run (no changes applied)', 'warning')}`);
  console.log('');
  ...
  const sourceFiles = files.filter((f) => f.type === 'file' && /\.(ts|js|tsx|jsx|json|md|css)$/i.test(f.name));

  console.log(`  ${colorText('Found', 'dim')} ${colorText(String(files.length), 'primary')} entries, ${colorText(String(sourceFiles.length), 'primary')} source files\n`);
  ...
  if (sourceFiles.length > 0) {
    ...
    for (const file of sourceFiles.slice(0, 5)) {
      ...
      if (content?.content) {
        ...
        console.log(`  ${colorText('📄', 'dim')} ${file.name} ${colorText(`(${lines} lines)`, 'muted')}`);
      }
    }
  }
  ...
  console.log(`  ${colorText('✓', 'success')} ${colorText('Analysis complete.', 'success')} ${colorText(String(sourceFiles.length) + ' files examined.', 'dim')}`);
  console.log(`  ${divider(48)}\n`);

  if (options.dryRun) {
    console.log(`  ${colorText('Dry-run mode:', 'warning')} ${colorText('no changes applied.', 'dim')}\n`);
  } else {
    console.log(`  ${colorText('→', 'dim')} ${colorText('To apply fixes, use the execution engine.', 'dim')}\n`);
  }
}
