...
function utilizationBar(percent: number, width = 15): string {
  ...
  const color = percent > 90 ? theme.colors.error
    : percent > 70 ? theme.colors.warning
    : percent > 40 ? theme.colors.primary
...
function savingsBar(savingsPercent: number, width = 10): string {
  ...
  const empty = width - filled;
  const color = savingsPercent > 50 ? theme.colors.success
    : savingsPercent > 25 ? theme.colors.primary
...
function safetyStatusColor(status: string): string {
  switch (status) {
    ...
    case 'throttled': return theme.colors.error;
...
export function renderContextPanel(display: ContextDisplay): string[] {
  ...
  // Header
  lines.push(`${theme.icon.bullet} ${headerColor}Context Engineering${theme.colors.text} ${display.core.active ? theme.colors.success + 'active' : theme.colors.textDim + 'inactive'}${theme.colors.text}`);

  if (!display.core.active && display.core.totalAssemblies === 0) {
    lines.push(`  ${theme.colors.textDim}No context data recorded yet. Start a workflow to build token optimization stats.${theme.colors.text}`);
    return lines;
  }

  // Core stats
  lines.push(`  ${theme.colors.textDim}Assemblies:${theme.colors.text} ${display.core.totalAssemblies}  ${theme.colors.textDim}Tokens Processed:${theme.colors.text} ${formatTokens(display.core.totalTokensProcessed)}  ${theme.colors.textDim}Total Savings:${theme.colors.text} ${formatTokens(display.core.totalSavings)}`);
  ...
  lines.push(`    ${theme.colors.textDim}Model Limit:${theme.colors.text} ${formatTokens(display.budget.modelMaxTokens)}  ${theme.colors.textDim}Allocated:${theme.colors.text} ${formatTokens(display.budget.allocated)}  ${theme.colors.textDim}Used:${theme.colors.text} ${formatTokens(display.budget.used)}  ${theme.colors.textDim}Available:${theme.colors.text} ${formatTokens(display.budget.available)}`);
  lines.push(`    ${theme.colors.textDim}Utilization:${theme.colors.text} ${utilizationBar(display.budget.utilizationPercent)}`);
  if (display.budget.overflowPrevented > 0) {
    lines.push(`    ${theme.icon.check} ${theme.colors.success}${display.budget.overflowPrevented} overflows prevented${theme.colors.text}`);
  }
  ...
  if (display.budget.providerBudgets.length > 0) {
    for (const pb of display.budget.providerBudgets) {
      ...
      const pbColor = pbPercent > 90 ? theme.colors.error : pbPercent > 70 ? theme.colors.warning : theme.colors.textDim;
      lines.push(`    ${theme.colors.textDim}${pb.provider}:${theme.colors.text} ${formatTokens(pb.used)} / ${formatTokens(pb.limit)} ${pbColor}(${pbPercent}%)${theme.colors.text}`);
    }
  }

  // --- Retrieval Section ---
  lines.push(`  ${theme.colors.textDim}File Retrieval:${theme.colors.text}`);
  const hitColor = display.retrieval.hitRatePercent > 50 ? theme.colors.success
    : display.retrieval.hitRatePercent > 20 ? theme.colors.warning
    : theme.colors.textDim;
  lines.push(`    ${theme.colors.textDim}Retrievals:${theme.colors.text} ${display.retrieval.totalRetrievals}  ${theme.colors.textDim}Files:${theme.colors.text} ${display.retrieval.selectedFiles} / ${display.retrieval.maxFiles}  ${theme.colors.textDim}Cache:${theme.colors.text} ${display.retrieval.cacheHits}h / ${display.retrieval.cacheMisses}m ${hitColor}(${display.retrieval.hitRatePercent}%)${theme.colors.text}`);
  ...
  if (display.retrieval.topFiles && display.retrieval.topFiles.length > 0) {
    ...
    for (const file of display.retrieval.topFiles.slice(0, 5)) {
      ...
      lines.push(`      ${bar} ${file.relevance.toFixed(0)}% ${theme.colors.textDim}${file.path}${theme.colors.text}`);
    }
  }
  ...
  if (display.compression.totalCompressions > 0) {
    ...
    lines.push(`    ${theme.colors.textDim}Compressions:${theme.colors.text} ${display.compression.totalCompressions}  ${theme.colors.textDim}Original:${theme.colors.text} ${formatTokens(display.compression.originalTokens)}  ${theme.colors.textDim}Compressed:${theme.colors.text} ${formatTokens(display.compression.compressedTokens)}`);
    lines.push(`    ${theme.colors.textDim}Savings:${theme.colors.text} ${savingsBar(display.compression.savingsPercent)}`);
    if (display.compression.staleEntriesCleaned > 0) {
      lines.push(`    ${theme.colors.textDim}Stale entries cleaned:${theme.colors.text} ${display.compression.staleEntriesCleaned}`);
    }
  }

  // --- Safety Section ---
  const sColor = safetyStatusColor(display.safety.status);
  lines.push(`  ${theme.colors.textDim}Context Safety:${theme.colors.text} ${sColor}${display.safety.status}${theme.colors.text}`);
  if (display.safety.overflowPrevented > 0 || display.safety.runawayDetections > 0 || display.safety.recursiveProtections > 0) {
    lines.push(`    ${theme.colors.textDim}Overflows prevented:${theme.colors.text} ${display.safety.overflowPrevented}  ${theme.colors.textDim}Runaway detections:${theme.colors.text} ${display.safety.runawayDetections}  ${theme.colors.textDim}Recursive protections:${theme.colors.text} ${display.safety.recursiveProtections}`);
  }
  if (display.safety.isThrottled) {
    lines.push(`    ${theme.icon.warning} ${theme.colors.warning}Context retrieval throttled${theme.colors.text}`);
  }
  ...
  if (display.longWorkflow.totalSummarizations > 0) {
    ...
    lines.push(`    ${theme.colors.textDim}Summarizations:${theme.colors.text} ${display.longWorkflow.totalSummarizations}  ${theme.colors.textDim}Idle Cleanups:${theme.colors.text} ${display.longWorkflow.idleCleanups}  ${theme.colors.textDim}Pruned:${theme.colors.text} ${display.longWorkflow.staleEntriesPruned}`);
    if (display.longWorkflow.isSummarized) {
      lines.push(`    ${theme.icon.check} ${theme.colors.success}Context summarized${theme.colors.text}`);
    }
  }
...
export function compactContextLine(display: ContextDisplay): string {
  ...
  const budgetColor = display.budget.utilizationPercent > 90 ? theme.colors.error
    : display.budget.utilizationPercent > 70 ? theme.colors.warning
    : theme.colors.success;
...
export function fileSelectionLine(selected: number, max: number): string {
  const color = selected > max * 0.9 ? theme.colors.warning
    : selected > max * 0.5 ? theme.colors.primary
    : theme.colors.success;
...
export function compressionSavingsLine(original: number, compressed: number): string {
  ...
  const color = percent > 50 ? theme.colors.success
    : percent > 25 ? theme.colors.primary
    : theme.colors.textDim;
...
export function safetyStatusLine(status: string): string {
  ...
  const icon = status === 'safe' ? theme.icon.check
    : status === 'caution' ? theme.icon.dot
    : theme.icon.warning;
